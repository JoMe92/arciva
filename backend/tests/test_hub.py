...
async def test_list_hub_assets_filtering(client, TestSessionLocal):
    async with TestSessionLocal() as session:
        ...
        asset1 = models.Asset(id=uuid.uuid4(), user_id=user_id, original_filename="a1.jpg", status=models.AssetStatus.READY, mime="image/jpeg", size_bytes=100)
        asset2 = models.Asset(id=uuid.uuid4(), user_id=user_id, original_filename="a2.jpg", status=models.AssetStatus.READY, mime="image/jpeg", size_bytes=100)
...
async def test_list_hub_date_buckets(client, TestSessionLocal):
    ...
    async with TestSessionLocal() as session:
        ...
        a1 = models.Asset(id=uuid.uuid4(), user_id=user_id, original_filename="2023.jpg", status=models.AssetStatus.READY, taken_at=d1, created_at=d1, mime="image/jpeg", size_bytes=100)
        l1 = models.ProjectAsset(project_id=proj.id, asset_id=a1.id, user_id=user_id)

        # Year 2024
        d2 = datetime(2024, 1, 15)
        a2 = models.Asset(id=uuid.uuid4(), user_id=user_id, original_filename="2024.jpg", status=models.AssetStatus.READY, taken_at=d2, created_at=d2, mime="image/jpeg", size_bytes=100)
