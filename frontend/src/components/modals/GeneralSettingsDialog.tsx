...
import {
  useExperimentalStorageSettings,
  useInvalidateExperimentalStorage,
} from '../../shared/settings/experimentalImageStorage'
...
const GeneralSettingsDialog: React.FC<GeneralSettingsDialogProps> = ({
  ...
  const storageStatusBadge = (status: ExperimentalStorageStatus) => {
    ...
    if (status === 'missing') {
      return (
        <span className="inline-flex items-center rounded-full bg-red-50 px-2 py-0.5 text-[11px] text-red-700">
          Not available
        </span>
      )
    }
    return (
      <span className="inline-flex items-center rounded-full bg-amber-50 px-2 py-0.5 text-[11px] text-amber-800">
        Not writable
      </span>
    )
  }
  ...
  const renderStorageLocations = () => {
    if (!experimentalStorage) {
      if (storageLoading) {
        return (
          <p className="text-[12px] text-[var(--text-muted,#6B645B)]">
            Loading experimental storage info…
          </p>
        )
      }
      ...
      return (
        <p className="text-[12px] text-[var(--text-muted,#6B645B)]">
          No storage configuration found yet.
        </p>
      )
    }
      ...
      return (
        <div className="space-y-2">
          <label className="text-[12px] font-semibold text-[var(--text,#1F1E1B)]">
            Current image storage path (experimental)
          </label>
          <div
          ...
          >
            ...
            <button
              type="button"
              onClick={() => handleCopyStoragePath(location.path)}
              className="text-[12px] font-medium text-[var(--primary,#A56A4A)]"
            >
              Copy
            </button>
          </div>
          <p className="text-[11px] text-[var(--text-muted,#6B645B)]">
            This local folder contains your photo storage (Uploads, Original, Export, etc.) for
            development.
          </p>
          <div className="flex items-center gap-2">
            {storageStatusBadge(location.status)}
            {location.message ? (
              <span className="text-[11px] text-red-600">{location.message}</span>
            ) : null}
          </div>
        </div>
      )
    ...
    return (
      <div className="space-y-2">
          ...
          {experimentalStorage.locations.map((location) => (
            ...
            >
              <div>
                ...
                {location.message ? (
                  <p className="text-[11px] text-red-600">{location.message}</p>
                ) : null}
              </div>
        ...
        <p className="text-[11px] text-[var(--text-muted,#6B645B)]">
          Multiple storage locations are currently used for development. This configuration is
          experimental and may not be supported in production.
        </p>
      </div>
    )
  }
  ...
  const bulkProgressPercent =
    bulkPhase === 'running' && bulkProgress.total > 0
    ...
    >
      <form id="general-settings-form" onSubmit={handleSubmit} className="space-y-6">
        <fieldset className="rounded-3xl border border-[var(--border,#E1D3B9)] bg-[var(--surface,#FFFFFF)] px-4 py-4">
          <legend className="px-1 text-[11px] font-semibold uppercase tracking-wide text-[var(--text-muted,#6B645B)]">
            Language
          </legend>
          <p className="text-[12px] text-[var(--text-muted,#6B645B)] mb-3">
            Choose the language used across menus and dialogs.
          </p>
          <select
            value={language}
        ...
        <fieldset className="space-y-3 rounded-3xl border border-[var(--border,#E1D3B9)] bg-[var(--surface,#FFFFFF)] px-4 py-4">
          ...
          <p className="text-[12px] text-[var(--text-muted,#6B645B)]">
            Download every project image as a ZIP archive organised in a {folderTemplate} folder
            hierarchy. Images are grouped by capture date when available.
          </p>
          <p className="text-[11px] text-[var(--text-muted,#6B645B)]">{basisDescription}</p>
          <p className="text-[11px] text-[var(--text-muted,#6B645B)]">
            This contains image files only—metadata/database exports live in their own workflow.
          </p>
          {bulkEstimate ? (
            <p className="text-[11px] text-[var(--text-muted,#6B645B)]">
              Estimated size: {bulkEstimate.totalFiles} images (~
              {formatBytes(bulkEstimate.totalBytes)}).
            </p>
          ) : null}
          <div className="flex flex-wrap items-center gap-3">
            ...
            {bulkPhase === 'running' ? (
              <button
                ...
                className="text-[11px] text-[var(--text-muted,#6B645B)] underline hover:text-[var(--text,#1F1E1B)]"
              >
                Cancel export
              </button>
            ) : null}
          ...
          {bulkPhase === 'running' ? (
            <p className="text-[11px] text-[var(--text-muted,#6B645B)]">
              You can close this dialog—the export keeps running in the background.
            </p>
          ) : bulkPhase === 'estimating' ? (
            <p className="text-[11px] text-[var(--text-muted,#6B645B)]">
              Gathering an estimate… hang tight.
            </p>
          ) : null}
          {bulkProgressLabel ? (
          ...
          {bulkResult && bulkPhase === 'success' ? (
            <div className="rounded-3xl border border-[var(--border,#E1D3B9)] bg-[var(--surface-subtle,#FBF7EF)] px-3 py-3">
              ...
              <p className="text-[12px] text-[var(--text-muted,#6B645B)]">
                {bulkResult.totalFiles} images packaged as a ZIP archive.
              </p>
        ...
        {experimentalUiEnabled ? (
          <section className="mt-8 space-y-4 border-t border-dashed border-red-200 pt-6">
            <div className="flex items-center gap-2">
              <h3 className="text-[13px] font-semibold text-[var(--text,#1F1E1B)]">
            ...
            <div className="space-y-4 rounded-3xl border border-red-200 bg-red-50/40 p-4">
              <div>
                <p className="text-[13px] font-semibold text-red-700">
                  ⚠ Experimental feature for local development only. Behavior may change or be
                  removed later.
                </p>
                <p className="text-[12px] text-[var(--text,#1F1E1B)]">
                  Configure where your local PhotoStore (Uploads, Original, Export, etc.) is stored
                  while testing on this machine.
                </p>
              </div>
              {storageSuccess ? (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[12px] text-emerald-700">
                  {storageSuccess}
                </div>
              ) : null}
              {experimentalStorage?.warning_active ? (
                <div className="rounded-2xl border border-red-200 bg-red-50 px-3 py-2 text-[12px] text-red-700">
                  Experimental storage configuration: one or more paths are not available. Some
                  images may be missing until all locations are connected.
                </div>
              ) : null}
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-[var(--text-muted,#6B645B)]">
                  Use this to test different local photo storage locations. This feature is intended
                  for developers only.
                </p>
      ...
      {experimentalUiEnabled ? (
        <ExperimentalImageStorageWizard
          open={storageWizardOpen}
          onClose={() => setStorageWizardOpen(false)}
          currentSettings={experimentalStorage}
          onSuccess={handleStorageWizardSuccess}
        />
      ) : null}
    </ModalShell>
  )
