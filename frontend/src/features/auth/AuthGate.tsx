...
export const AuthGate: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-subtle,#FBF7EF)] px-4 py-10 text-[var(--text,#1F1E1B)]">
      <div className="mx-auto flex min-h-[80vh] max-w-md flex-col items-center justify-center gap-8">
        <StoneTrailLogo slogan="Photo archive for humans." />
        <form
          onSubmit={handleSubmit}
          className="w-full rounded-3xl border border-[var(--border,#E1D3B9)] bg-[var(--surface,#FFFFFF)] p-6 shadow-[0_20px_60px_rgba(31,30,27,0.12)]"
        >
          <div className="mb-4">
            <h1 className="text-xl font-semibold tracking-tight">{heading}</h1>
            <p className="text-sm text-[var(--text-muted,#6B645B)]">
              {mode === 'login' ? 'Access your local Arciva library.' : 'Choose a strong password (8+ characters).'}
            </p>
          </div>
