...
// Ideally these would be shared, but for safety in this task we duplicate local types.
...
function AssetTile({ tile, viewMode }: { tile: HubTile; viewMode: HubViewMode }) {
    ...
    const content = (
        <div
            className={`relative w-full overflow-hidden rounded border border-[var(--border,#E1D3B9)] group cursor-default`}
        >
            <img
    ...
    const listContent = (
        <div
            className={`flex items-center gap-3 rounded border px-3 py-2 border-[var(--border,#E1D3B9)]`}
        >
            <img
                src={primary.thumb_url ?? ''}
                alt=""
...
export default function ImageHub() {
    ...
    // Reset asset list when tab changes
    useEffect(() => {
        // Logic for automatic selection of first project
    }, [tab])
        ...
        <div className="flex h-screen flex-col bg-[var(--background,#FBF7EF)] text-[var(--text,#1F1E1B)] transition-colors duration-300">
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden p-4">
                ...
                <div className="flex flex-wrap gap-4 text-xs">
                    {/* Filter Controls (Type, Rating, Label, Date Range) - Simplified from ImportPane */}
                    ...
                    {/* ... other filters omitted for now or copied ... */}
                </div>
